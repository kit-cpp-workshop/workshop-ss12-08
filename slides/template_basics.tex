...
\begin{frame}[t, fragile]{intricacies: dependent names, typename}
	...
	\begin{block}{keyword `typename` (Standard, 14.6/2-3)}
		Wird ein Typ mittels \emph{qualified-id} benannt (= es taucht ein \verb|::| auf), und ist taucht ein template Parameter (auch implizit) auf, so muss vor den ganzen Ausdruck ein \verb|typename| gestellt werden.

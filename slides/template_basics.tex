...
\begin{frame}[fragile]{intricacies: typename, template, dependent names}
	\begin{enumerate}
		\item Generell eine gute Idee: alles qualifiziert (explizit) hinschreiben, d.h. \verb|mynamespace::MyClass::MyStuff| \\
		      Bei templates ist dies extrem wichtig!
		\item Folgt auf ein \verb|.| \verb|->| oder \verb|::| der Name eines templates, so muss vor diesem namen ein \verb|template| eingefügt werden.
		\item Wird ein Typ mittels \emph{qualified-id} benannt (= es taucht ein \verb|::| auf), so muss vor den ganzen Ausdruck ein \verb|typename| gestellt werden.
	\end{enumerate}
\end{frame}
